...
GLSL.Program.prototype = {
    ...
    runProgram: function(env)
    {
        ...
        if (!environment.validateForShader(this.vertexShader))
            throw new Error("Couldn't run shader program: vertex shader environment validation failed.");

        if (!environment.validateForShader(this.fragmentShader))
            throw new Error("Couldn't run shader program: vertex shader environment validation failed.");
        ...
        try {
            env.enterRunScope();
            this.vertexShader.executable.code.call(null, GLSL_RT, env);

...
CodeGenerator.Callbacks[ASTNode.Types.BinaryExpression] = function(node)
{
    ...
    */
    ...
    if (do_assign) {
        var builder = new AssignmentBuilder(node.left);
...
AssignmentBuilder = function(expr) {
    ASTVisitor.call(this);
...
AssignmentBuilder.prototype = {
    constructor: AssignmentBuilder,
    __proto__: ASTVisitor.prototype,
    ...
    visitorForType: function(type)
    {
        if (type in AssignmentBuilder.Callbacks)
            return AssignmentBuilder.Callbacks[type];

        throw new Error("Unexpected AST node encountered by assignment builder.");
    },
}

AssignmentBuilder.Callbacks = {};

AssignmentBuilder.Callbacks[ASTNode.Types.IntegerLiteral] = function(node)
{
    return this.params.push(node.value);
}

AssignmentBuilder.Callbacks[ASTNode.Types.FloatLiteral] = function(node)
{
    return this.params.push(node.value);
}

AssignmentBuilder.Callbacks[ASTNode.Types.Identifier] = function(node)
{
    return this.operand = node.name;
}

AssignmentBuilder.Callbacks[ASTNode.Types.Operator] = function(node)
{
    if (node.operator instanceof ASTNode)
        this.visitNode(node.operator);
}

AssignmentBuilder.Callbacks[ASTNode.Types.PostfixExpression] = function(node)
{
    // {operator: Node.Operator, expression: Node.$expression}
...
AssignmentBuilder.Callbacks[ASTNode.Types.IndexSelector] = function(node)
{
    // {index: Node.$expression}
    this.visitNode(node.index);
}

AssignmentBuilder.Callbacks[ASTNode.Types.FieldSelector] = function(node)
{
    // {selection: string}

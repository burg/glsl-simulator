...
CodeGenerator.prototype = {
    ...
    translateShader: function() {
        ...
        this._addLine("");
        this._addLine(this._makeName("main") + "();");
        this._addLine("");
        var executable = {};
        try {
...
CodeGenerator.Callbacks[ASTNode.Types.FunctionCall] = function(node)
{
    ...
    var callee = this._makeName(node.function_name);
    if (node.function_name in Builtins)
        callee = "RT." + node.function_name;
    if (node.function_name in constructorMap)
        callee = "RT." + constructorMap[node.function_name];
...
CodeGenerator.Callbacks[ASTNode.Types.FunctionDeclaration] = function(node)
{
    ...
    this._addLine("var " + this._makeName(node.name) + " = function(" + paramNames.join() + ") {");
...
{
    // {name: string}
    return this._makeName(node.name);
}
...
CodeGenerator.Callbacks[ASTNode.Types.BinaryExpression] = function(node)
{
    ...
    // TODO: move the runtime op_ functions to be static members of GLSL.Runtime.Op.
    ...
    */
    ...
    var result = (func) ? ("RT." + func + "(" + left + ", " + right + ")") : right;
